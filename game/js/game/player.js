	...
	Player.prototype.init = function (playerLayer, playerGroup, foreground, playground) {
		...
		foreground.on('mousemove', function (event) {
			if (!self.paused && !self.disabled) {
				...
				var rect = self.foreground.getCanvas()._canvas.getBoundingClientRect();
		...
		// Also bind to playground for better coverage
		playground.on('mousemove', function (event) {
			if (!self.paused && !self.disabled) {
				var rect = playground[0].getBoundingClientRect();
				var x = event.clientX - rect.left;
				var y = event.clientY - rect.top;
		...
		foreground.on('click', function (event) {
			console.log('Foreground click detected, paused:', self.paused, 'disabled:', self.disabled);
			if (!self.paused && !self.disabled) {
				// Get mouse coordinates relative to the canvas
				var rect = self.foreground.getCanvas()._canvas.getBoundingClientRect();
				var x = event.clientX - rect.left;
		...
		// Also bind click to playground for better coverage
		playground.on('click', function (event) {
			console.log('Playground click detected, paused:', self.paused, 'disabled:', self.disabled);
			if (!self.paused && !self.disabled) {
				var rect = playground[0].getBoundingClientRect();
				var x = event.clientX - rect.left;
				var y = event.clientY - rect.top;
				console.log('Shooting at (playground):', x, y);
