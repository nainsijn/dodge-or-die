...
define(function () {
	...
	Enemy.prototype.init = function (enemyLayer, enemyGroup) {
		...
		// add damage animation tween (reusable)
	...
	Enemy.prototype.showDeath = function () {
		...
		// Show blood instantly, then hide it very quickly
		this.blood.setOpacity(0.7);
		this.blood.setRadius(20);

		// Use setTimeout for immediate cleanup instead of tween
		setTimeout(function () {
			self.blood.setOpacity(0);
			self.blood.setRadius(12); // reset to original size
		}, 50); // 50ms flash
	...
	Enemy.prototype.goTo = function (target) {
		...
		// Cache current position for performance
		var currentX = enemy.getX();
		var currentY = enemy.getY();
		...
		// Pre-calculate movement direction for performance
		var moveX = (deltaX / distance) * self.speed;
		var moveY = (deltaY / distance) * self.speed;

		// rotate towards the target (simplified rotation calculation)
		var angle = Math.atan2(deltaY, deltaX);
		enemy.setRotation(angle - Math.PI / 2); // -90 degrees adjustment
		blood.setRotation(angle - Math.PI / 2);

		// Optimized movement animation with reduced calculations
		this.moveAnim = new Kinetic.Animation(function () {
			...
			// Quick distance check using squared distance (avoid sqrt for performance)
			var distanceSquared = deltaX * deltaX + deltaY * deltaY;

			if (distanceSquared > E * E) {
				// Use pre-calculated movement direction, recalculate only if needed
				if (Math.abs(deltaX) > 0.1 || Math.abs(deltaY) > 0.1) {
					var distance = Math.sqrt(distanceSquared);
					moveX = (deltaX / distance) * self.speed;
					moveY = (deltaY / distance) * self.speed;
				}

				// Apply movement
				enemy.move(moveX, moveY);
				blood.move(moveX, moveY);
			} else {
				// Target reached, start attacking
				enemy.setAnimation('attack');
				self.moveAnim.stop();
				if (self.attackCallback) {
					// attack in intervals based on speed
					var time = 150 + 1500 / self.speed;
					var attack = function () {
						if (!self.paused) {
